...
def sort_suffix(a,b):
    ...
    return sg[a+pos] - sg[b+pos]    


def lrs ():
    
    suffix = sorted(sg[i:] for i,_ in enumerate(sg)) # too slow for moby dick file ~1.2 m chars and worst, we get a kill from s.o. because of memory usage
    #suffix = [s[i:] for i,_ in enumerate(s)]
    #suffix.sort()
    #print (suffix)
    #print (len(suffix))

    #suffix = [i for i in range(0,len(sg))] # pointer like to the original string, but still slow. 
    ...
    while i < slen-1:
        ...
        if lrs_len > max_len:
            max_len = lrs_len
            lrs_str = sg[suffix[i]:suffix[i]+lrs_len]
...
if __name__ == "__main__":
    ...
    sg.frombytes(chars.encode())
    print ("input ok - #",len(chars)," chars read")
    print (lrs().tobytes().decode())
